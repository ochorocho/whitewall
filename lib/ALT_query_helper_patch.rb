...
module QueryHelperPatch
  ...
  def column_content_with_editior_id(column, issue)
    value = column.value(issue)
    if %w(Fixnum Float).include?( value.class.name ) and [:editor_id].include?(column.name)
      sprintf "%.2f", value
    else
      column_content_without_editior_id(column, issue)
    end
  end
